...
namespace Asuna
{
  ...
  ProgramObject::createShader(const std::string &text, GLenum shaderType)
  {
  ...
  ProgramObject::createVertexShader(const string& absolutePath)
  {
    m_shaders[ASUNA_VERTEX_SHADER] = createShader(loadShader(absolutePath), GL_VERTEX_SHADER);
    glAttachShader(m_program, m_shaders[ASUNA_VERTEX_SHADER]);
  }

  ProgramObject::createFragmentShader(const string& absolutePath)
  {
    m_shaders[ASUNA_FRAGMENT_SHADER] = createShader(loadShader(absolutePath), GL_FRAGMENT_SHADER);
    glAttachShader(m_program, m_shaders[ASUNA_FRAGMENT_SHADER]);
  }

  ProgramObject::deleteVertexShader()
  {
    glDetachShader(m_program, m_shaders[ASUNA_VERTEX_SHADER]);
    glDeleteShader(m_shaders[ASUNA_VERTEX_SHADER]);
  }

  ProgramObject::deleteFragmentShader()
  {
    glDetachShader(m_program, m_shaders[ASUNA_FRAGMENT_SHADER]);
    glDeleteShader(m_shaders[ASUNA_FRAGMENT_SHADER]);
  }

  ProgramObject::update(const Transform& transform, const Camera& camera)
  {
  ...
  ProgramObject::loadShaderFromFile(const int &absolutePath)
  {
    std::ifstream file;
  ...
  ProgramObject::checkProgramError(GLuint program, GLuint flag, const std::string& errMessage)
  {
    GLint success = 0;
    GLchar error[1024] = {0};
  ...
  ProgramObject::checkProgramError(GLuint shader, GLuint flag, const std::string& errMessage)
  {
    GLint success = 0;
    GLchar error[1024] = {0};
