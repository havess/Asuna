...
Shader::Shader(const std::string& filename){
    ...
    m_shaders[0] = createShader(loadShader("./shaders/" + filename + ".vs"), GL_VERTEX_SHADER);
    m_shaders[1] = createShader(loadShader("./shaders/" + filename +".fs"), GL_FRAGMENT_SHADER);
...
static void checkShaderError(GLuint shader, GLuint flag, bool isProgram, const std::string& errMessage){
    ...
    std::cout << "no shader error" << std::endl;
}
...
static GLuint createShader(const std::string& text, GLenum shaderType){
    ...
    std::cout << "shader was created" << std::endl;
    return shader;
}
